...
//                        $q4 = get_string('notapplicable', 'questionnaire'); DEV JR choose one solution please
	...
	// ************************************************************************************************
    // devjr original D.J. NOVEMBER 2010 modified DEC 2011 
    function survey_results_navbar_alpha($curr_rid, $groupid, $cm, $byresponse) {
        ...
        if (!$responses = get_records_sql($sql)) {
        	return;
        }
        ...
        if (!$byresponse) { // display single response + navbar
	        ...
	        if ($prev_rid != null) {
	        	...
	        	array_push($linkarr, '<b><<</b> <a href="'.$url.'&amp;rid='.$first_rid.'" title="'.$firstuserfullname.'">'.get_string('firstpage','questionnaire').'</a>');
	        ...
	        if ($next_rid != null) {
	            ...
	            array_push($linkarr, '<a href="'.$url.'&amp;rid='.$last_rid.'"  title="'.$lastuserfullname .'">'.get_string('lastpage','questionnaire').'</a>&nbsp;<b>>></b>');
	        }
